...
export const getTopicById = async (req, res) => {
    try {
        ...
        const topicProgress = userProgress.topics.find((t) => t.topicId?.toString() === topic._id.toString()) || {
            doneQuestions: [],
            bookmarkedQuestions: [],
            notes: [],
        };
        ...
        const notesMap = new Map((topicProgress.notes || []).map((n) => [n.questionId?.toString(), n.text]));
...
export const updateFields = async (req, res) => {
    try {
        ...
        let progress = await Progress.findOne({ userId }).lean().exec();
        if (!progress) {
        ...
        if (!topicExists) {
            await Progress.updateOne({ userId }, { $addToSet: { topics: { topicId, doneQuestions: [], bookmarkedQuestions: [], notes: [] } } });
        }
        ...
        if (field === "Done" || field === "Bookmark") {
            ...
            const setResult = await Progress.updateOne(topicMatch, { $set: { "topics.$[topic].notes.$[note].text": value } }, {
        ...
        progress = await Progress.findOne({ userId }).lean().exec();
